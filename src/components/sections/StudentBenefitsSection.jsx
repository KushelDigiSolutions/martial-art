...
export default function StudentBenefitsSection() {
  return (
    <section className="bg-brand-darker text-white py-16 sm:py-24 lg:py-32 relative border-b border-brand-gray">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {benefits.map((benefit, index) => {
            ...
            return (
              <div 
                key={index} 
                className="group relative bg-[#0a0a0a] border border-brand-gray p-6 md:p-12 transition-all duration-500 hover:-translate-y-4 hover:border-brand-red cursor-pointer overflow-hidden"
              >
                {/* Glowing red accent hidden until hover */}
                ...
                <IconComponent className="w-12 h-12 text-brand-red mb-8 transform transition-transform duration-500 group-hover:scale-110 group-hover:rotate-12" />
                
                <h3 className="text-2xl font-black mb-6 tracking-wide font-display uppercase">{benefit.title}</h3>
                
                <p className="text-gray-400 leading-relaxed font-sans font-light text-base">
                  {benefit.description}
                </p>
              </div>
            );
